...
def argparse_datatree(args, run_config, run_config_es):
    ...
    ## INTERFACE Get all surface data and merge in one dataframe, adding parameters and computing steady state time
        ...
        treat_data_surface(
            "up", modifier=args.modifier, folder_path=run_config["folder_path"]
        )
        steadyState_data_surface(
            interface="up", modifier="_no_tail", folder_path=run_config["folder_path"]
        )
    ...
    # 2D GAS Get all surface 2D data and merge in one dataframe, adding parameters and computing steady state time
        ...
        treat_data_surface("2D", folder_path=run_config["folder_path"])
    ## Prepare data for SINDy
        ...
        sindy_data(
            ...
            scaling=False,
            modifier="_no_tail",
            folder_path=run_config["folder_path"],
        )
    else:
        print(f"Unknown task: {args.task}")


def argparse_plottree(args, run_config, run_config_es):
    ## Load and plot data for SR
    if args.task == "load_plot_data_sr":
        load_plot_data_sr(
            run_config["save_folder_path"],
            modifier="_no_tail",
            folder_path=run_config["folder_path"],
        )
    ...
    if args.task == "insert_derivatives_dataframe":
        ...
        plot_derivative(df_surface, run_config["save_folder_path"])


def argparse_symbolicRegressiontree(args, run_config, sr_config, run_config_es):
    ...
    if args.task == "run_symbolic_regression":
        sr_model = symbolic_regression(
            ...
            units="h",
            run_graph=False,
            save_path=run_config["save_folder_path"],
    ...
    if args.task == "run_sr_conductivity":
        ...
        sr_model = symbolic_regression_conductivity(
            run_config["save_folder_path"], folder_path=run_config["folder_path"]
        )
        equation_file_path = sr_model.equation_file_.replace(".csv", "") + "_table.txt"
        with open(equation_file_path, "w") as file:
            file.write(
...
def argparse_sindy(args, run_config, sr_config, sindy_config, run_config_es):
    ...
    if args.task == "run_sindy_conductivity":
        ...
        model_data = sindy_surface(
            ...
            save_path=run_config["save_folder_path"] + "\\sindy_results",
            model_path=None,
    ...
    if args.task == "run_sindy_gridSearch":
        grid_search_sindy(
            run_config["save_folder_path"] + "\\sindy_results",
